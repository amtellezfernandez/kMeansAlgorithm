...
def kmeansPlusPlusInit(points: RDD[(Double, Double)], K: Int): Array[(Double, Double)] = {
  // Choose the first centroid randomly from the data points
  val firstCentroid = points.takeSample(false, 1)(0)
  ...
  for (_ <- 1 until K) {
    ...
    // Compute squared distances from each point to its closest centroid
    val distances = points.map { point =>
      val closestDist = bcCentroids.value.map(centroid => distanceSquared(point, centroid)).min
      (point, closestDist)
    }.collect()

    // Calculate the cumulative distribution of distances
    val cumulativeDistances = distances.scanLeft(0.0)((acc, elem) => acc + elem._2)
    val cumulativeDistancesWithPoints = cumulativeDistances.zip(distances.map(_._1))

    // Generate a random number for selecting the next centroid
    val rand = random * cumulativeDistancesWithPoints.last._1
    val nextCentroid = cumulativeDistancesWithPoints.find(_._1 >= rand).get._2

    centroids = centroids :+ nextCentroid
    bcCentroids.unpersist()
  }

  centroids
}

// Initialize K random points as cluster centers using k-means++
var kPoints = kmeansPlusPlusInit(pointsRDD, K)
println("Initial K Center points using k-means++:")
...
// Variable to store iteration times
val iterationTimes = ListBuffer.empty[(Int, Long)]

// K-means algorithm loop
var tempDist = Double.PositiveInfinity
var iteration = 0
var totalIterationTime = 0L  // Variable to accumulate total iteration time
...
while (tempDist > convergeDist) {
  ...
  val iterationTime = endTime - startTime  // Compute iteration time
  iterationTimes.append((iteration, iterationTime))  // Store iteration time
  totalIterationTime += iterationTime  // Accumulate total iteration time

  println(s"Distance between iterations: $tempDist")
  println(s"Iteration time: ${iterationTime} ms")  // Print iteration time
...
val timingsDF = Seq(
  ("Iteration Time (total)", totalIterationTime),
